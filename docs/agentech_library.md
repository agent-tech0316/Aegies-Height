...
Important: `Agentech.forward()` is a real robot command by default. If the FF SDK is installed and the machine can reach the robot, it connects to the normal robot hotspot host (`192.168.234.1`) with the Aegis EDU / Ultra variant (`zsl-1`), stands the robot with `motion.stand()`, waits five seconds for the body to fully stand, sends `motion.cmd_vel(linear=+speed, angular=0.0)`, waits for the requested time, then stops. Use `dry_run=True` only when you intentionally want practice code that does not move hardware.
...
with Agentech.robot(host="192.168.234.1", dry_run=False) as dog:
    ...
    dog.backward(speed=0.2, seconds=1.0)
    dog.lateral_left(speed=0.2, seconds=1.0)
    dog.lateral_right(speed=0.2, seconds=1.0)
    dog.left(angle=45)
...
| --- | --- | --- | --- |
| `stand` | `Agentech.stand(stand_wait=5)` | `stand_wait=5.0` | Stands the dog up and waits so it is ready to move. |
...
| `lateral_left` | `Agentech.lateral_left()` | `speed=0.2`, `seconds=1.0` | Walks sideways to the robot's left using FF SDK lateral velocity. |
| `lateral_right` | `Agentech.lateral_right()` | `speed=0.2`, `seconds=1.0` | Walks sideways to the robot's right using FF SDK lateral velocity. |
| `turn_left` | `Agentech.turn_left()` | `angle=45`, `speed=0.35` | Turns left by degrees. |
| `turn_right` | `Agentech.turn_right()` | `angle=45`, `speed=0.35` | Turns right by degrees. |
| `twist_left` | `Agentech.twist_left()` | `angle=28`, `speed=0.35` | Small fixed-foot left body twist previewed with the twist-left MuJoCo clip. |
| `twist_right` | `Agentech.twist_right()` | `angle=28`, `speed=0.35` | Small fixed-foot right body twist previewed with the twist-right MuJoCo clip. |
...
| `get_battery_status` | `Agentech.get_battery_status()` | none | Reads the battery state for the session panel. |
...
| `speed` | `lateral_left`, `lateral_right` | `0.2` | `0.0` to `0.78` | Sideways speed in meters per second. |
| `seconds` | walking commands | `1.0` | `0.0` to `10.0` | How long to hold the command. |
| `stand_wait` | `stand`, all movement commands | `5.0` | `0.0` to `10.0` | How long to wait after automatic stand before motion starts. |
| `angle` | `turn_left`, `turn_right`, `twist_left`, `twist_right`, `look_up`, `look_down` | `45` for turns, `28` for twist, `10` for tilt | turns/twist: `-360` to `360`; look up/down: `0` to `25` | Human-readable degrees, capped so the preview and real command stay realistic. |
| `speed` | `turn_left`, `turn_right`, `twist_left`, `twist_right` | `0.35` | `0.05` to `2.09` | Yaw rate used to estimate turn or twist duration. |
| `speed` | `look_up`, `look_down` | `0.12` | `0.03` to `0.5` | Pitch velocity in radians per second. |
...
| Look up / forward pitch | `0` to `25 deg` |
| Look down / backward pitch | `0` to `25 deg` |
...
| `aegis.walk_lateral_left` | `Agentech.lateral_left(speed, seconds)` | `motion.cmd_vel(linear=0.0, lateral=+speed, angular=0.0)` |
| `aegis.walk_lateral_right` | `Agentech.lateral_right(speed, seconds)` | `motion.cmd_vel(linear=0.0, lateral=-speed, angular=0.0)` |
...
| `aegis.twist_left` | `Agentech.twist_left(angle)` | fixed-foot twist-left preview, FF SDK angular velocity command |
| `aegis.twist_right` | `Agentech.twist_right(angle)` | fixed-foot twist-right preview, FF SDK angular velocity command |
...
| `aegis.get_status` | `Agentech.get_battery_status()` | `state.battery()` |
